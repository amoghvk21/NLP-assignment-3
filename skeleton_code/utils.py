...
def viterbi(observations, num_states, transition_prob, emission_prob):
    ...
    # Vectorized Viterbi algorithm using matrix operations
    # Store path as a matrix during computation for efficiency
    path_matrix = np.zeros((num_states, len(observations)), dtype=int)
    
    # Vectorized computation for all time steps
    for t in range(1, len(observations)):
        # Use reshape for broadcasting V[:, t-1] to (num_states, 1)
        # prob of being in a state * transition from that state to the next state
        prev_probs = V[:, t - 1].reshape(-1, 1) * transition_prob  # (num_states, num_states)
        
        # Get emission probabilities for current observation shape (num_states,)
        emit = emission_prob[:, observations[t]]

        # Use reshape for broadcasting emit as (1, num_states) if needed
        # Reshape into row vector  (num_states, ) - implicit so not really needed
        all_probs = prev_probs * emit.reshape(1, -1)    # (num_states, num_states)
        
        # Column-wise max and argmax operations
        V[:, t] = np.max(all_probs, axis=0)    # best probs become column in V
        path_matrix[:, t] = np.argmax(all_probs, axis=0) # want to know which state got you the best path
    
    # Convert path_matrix to dictionary format for backtracking (vectorized conversion)
    for t in range(1, len(observations)):
        for y in range(num_states):
            path[(y, t)] = path_matrix[y, t]
...
def calculate_variation_of_information(U, V):
    ...
    # Handle division by zero: when both entropies are 0, normalized VI should be 0
    if entropy_U + entropy_V == 0:
        normalized_vi = 0.0  # Perfect agreement (no variation)
    else:
        normalized_vi = variation_of_information / (entropy_U + entropy_V)
    
    return variation_of_information, normalized_vi
